...
const admin = require('firebase-admin'); // Requiring needed files


const helpers = require('./helpers');

const spotifyService = require('./spotify-service'); // Initializing the Firebase admin app
...
const questionsRef = ref.child('artistQuestions'); // Main function

exports.dialogflowFirebaseFulfillment = functions.https.onRequest((req, res) => {
  // From the Spotify Artist object, we can now format the fulfillment
  // with needed data
  const getArtistInfos = artist => {
    const answer = helpers.answerConstruction(artist); // We're saving the artist ID to Firebase
  ...
  const getActions = action => {
    ...
    questionsRef.once('value').then(snapshot => {
      const id = snapshot.val().artistId;
  ...
  const actions = req.body.queryResult.parameters.moreInfos;
  const artistName = req.body.queryResult.parameters.artist; // If we've got an artistName, we need to find it on Spotify
